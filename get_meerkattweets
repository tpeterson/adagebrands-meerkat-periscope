...
meerkatters_file = "brand_meerkatters.txt"
...
brand_names.each do |brand|
  ...
  tweets.each do |tweet|
    if tweet["entities"]["urls"].empty? == false
      ...
      if /^mrk.tv/.match(link)
        puts "Meerkat link: #{link}"
        mrkt_links.push(
          name: name,
          link: link,
          id: id
...
File.open(meerkatters_file, 'w') do |f|
  f.puts JSON.pretty_generate(mrkt_links)
end
